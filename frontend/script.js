...
(function () {
  ...
  function attemptLogin(username, password) {
    return fetch(backendHttpUrl() + "login", {
      ...
      if (res.status === 404)
        throw new Error(
          "Login is not deployed on the backend yet. Deploy the latest backend commit on Render.",
        );
      if (!res.ok)
        throw new Error(
          "The server could not complete the login (HTTP " + res.status + ").",
        );
      return res.json();
    });
  }
  ...
  const TEAM_MEMBERS = [
    {
      name: "Anamitra Laha",
      email: "anamitra.laha2007@gmail.com",
      role: "Add role here",
    },
    {
      name: "Rupam Sasmal",
      email: "sasmalrupam2417@gmail.com",
      role: "Add role here",
    },
    {
      name: "Biswajit Dey",
      email: "deybiswajit2005@gmail.com",
      role: "Add role here",
    },
    {
      name: "Pratyush Chakarborty",
      email: "pratyushchak06@gmail.com",
      role: "Add role here",
    },
  ];
  ...
  const MATERIAL_DISPLAY = {
    HDPE: {
      ...
      itemColor:
        "linear-gradient(160deg, #e7ecff 0%, #adc6ff 55%, #7ba2f0 100%)",
      icon: "recycling",
    },
    PET: {
      ...
      itemColor:
        "linear-gradient(160deg, #f2f6ff 0%, #cfe0ff 60%, #9fc0ff 100%)",
      icon: "local_drink",
    },
    PP: {
      ...
      itemColor:
        "linear-gradient(160deg, #fff2df 0%, #ffd9a3 55%, #ffb95f 100%)",
      icon: "package_2",
    },
    LDPE: {
      ...
      itemColor:
        "linear-gradient(160deg, #f5f7ff 0%, #dfe6f5 55%, #b9c3d6 100%)",
      icon: "package_2",
    },
    PVC: {
      ...
      itemColor:
        "linear-gradient(160deg, #fdeeee 0%, #f2b8b8 55%, #d97575 100%)",
      icon: "warning",
    },
    PS: {
      ...
      itemColor:
        "linear-gradient(160deg, #f7f7f7 0%, #dcdcdc 55%, #b0b0b0 100%)",
      icon: "delete_forever",
    },
    OTHER: {
      ...
      itemColor:
        "linear-gradient(160deg, #4a4f5c 0%, #33394a 60%, #23283a 100%)",
      icon: "delete_forever",
    },
  };
  ...
  function collectEls() {
    els = {
      ...
      nirBars: document.getElementById("nirBars")
        ? document.getElementById("nirBars").children
        : [],
      ...
      // Settings page
      settingsConnectionBadge: document.getElementById(
      ...
      settingsRefreshHealthBtn: document.getElementById(
        "settingsRefreshHealthBtn",
      ),
      settingsCollapsedCountInput: document.getElementById(
        "settingsCollapsedCountInput",
      ),
      settingsExportBtn: document.getElementById("settingsExportBtn"),
      settingsClearHistoryBtn: document.getElementById(
  ...
  function getDailyStats() {
    try {
      const saved = JSON.parse(
        localStorage.getItem(DAILY_STATS_STORAGE_KEY) || "null",
      );
      if (
        saved &&
        saved.day === localDayKey() &&
        Number.isFinite(saved.total) &&
        Number.isFinite(saved.recyclable) &&
        Number.isFinite(saved.confidenceSum)
      ) {
        return saved;
      }
    } catch (error) {
  ...
  function renderDailyStats() {
    ...
    const recyclablePct = stats.total
      ? Math.round((stats.recyclable / stats.total) * 100)
      : 0;
    const averageConfidence = stats.total
      ? roundTo(stats.confidenceSum / stats.total, 1)
      : 0;

    if (els.itemsSortedCount)
      els.itemsSortedCount.textContent = stats.total.toLocaleString();
    if (els.recyclablePct) els.recyclablePct.textContent = recyclablePct + "%";
    if (els.modelAccuracy)
      els.modelAccuracy.textContent = averageConfidence.toFixed(1) + "%";
  }
  ...
  function buildHistoryRow(entry) {
    ...
    tr.innerHTML =
      ...
      (entry.recyclable ? "text-primary" : "text-error") +
      ' text-lg">' +
      entry.icon +
      "</span>" +
      "</div>" +
      '<span class="font-body-md font-medium">' +
      entry.material +
      "</span>" +
      "</div>" +
      "</td>" +
  ...
  function renderHistoryTable() {
    ...
    const rows = showFullHistory
      ? historyLog
      : historyLog.slice(0, HISTORY_COLLAPSED_COUNT);
    rows.forEach((entry) =>
      els.historyTableBody.appendChild(buildHistoryRow(entry)),
    );

    if (els.viewFullLogBtn) {
      const label = els.viewFullLogBtn.querySelector("span:first-child");
      if (label)
        label.textContent = showFullHistory ? "Show Recent" : "View Full Log";
    }
  }
  ...
  function applyBackendResult(data) {
    ...
    const arrow =
      route === "LEFT" ? "west" : route === "RIGHT" ? "east" : "south";
    ...
    if (recyclable) {
      ...
      if (els.carbonOffset)
        els.carbonOffset.parentElement.style.display = "none";
    }
    ...
    setTimeout(() => {
      ...
      if (display.peakPos) {
        ...
        els.spectralPeakTitle.textContent =
          "Peak identified at " + display.peakLabel;
      } else {
    ...
    els.pneumaticsStatus.textContent = nominal
      ? "PNEUMATICS OK"
      : "PNEUMATICS WARN";
  ...
  function showCameraFallback(label) {
    ...
    if (els.camStatusBadge)
      els.camStatusBadge.textContent = label || "CAM OFFLINE";
  }
  ...
  function setConnectionStatus(connected) {
    if (els.systemStatusLabel) {
      els.systemStatusLabel.textContent = connected
        ? "Scanning Active"
  ...
  function connectWebSocket() {
    ...
    socket = new WebSocket(
      BACKEND_WS_URL + "?token=" + encodeURIComponent(token),
    );
    ...
    socket.onmessage = (event) => {
      ...
      try {
        ...
        console.error(
          "SpectraLink: could not parse message from backend",
          event.data,
        );
        return;
      }
  ...
  function getFilteredHistory() {
    const query = (
      (els.historySearchInput && els.historySearchInput.value) ||
      ""
    )
      .trim()
      .toLowerCase();
    const materialFilter = els.historyMaterialFilter
      ? els.historyMaterialFilter.value
      : "ALL";
    const decisionFilter = els.historyDecisionFilter
      ? els.historyDecisionFilter.value
      : "ALL";

    return historyLog.filter((entry) => {
      if (materialFilter !== "ALL" && entry.material !== materialFilter)
        return false;
  ...
  function renderHistoryPage() {
    ...
    rows.forEach((entry) =>
      els.historyPageTableBody.appendChild(buildHistoryRow(entry)),
    );

    if (els.historyResultCount) {
      els.historyResultCount.textContent =
  ...
  function wireHistoryPage() {
    [
      ...
      els.historyMaterialFilter,
      els.historyDecisionFilter,
    ].forEach((el) => {
      if (!el) return;
  ...
  function initCharts() {
    ...
    const byMaterialCanvas = document.getElementById(
      "confidenceByMaterialChart",
    );
    if (byMaterialCanvas) {
  ...
  function updateAnalytics() {
    ...
    if (els.analyticsSampleSize) {
      els.analyticsSampleSize.textContent =
        total + (total === 1 ? " scan this session" : " scans this session");
    }
    ...
    historyLog.forEach((entry) => {
      ...
      if (!confidenceByMaterial[entry.material])
        confidenceByMaterial[entry.material] = [];
    ...
    if (charts.material) {
      ...
      charts.material.data.datasets[0].backgroundColor = materials.map(
        (m) => MATERIAL_CHART_COLORS[m] || "#8c909f",
      );
      charts.material.update();
    }

    if (charts.decision) {
      charts.decision.data.datasets[0].data = [
        recyclableCount,
    ...
    if (charts.confidenceByMaterial) {
      ...
      charts.confidenceByMaterial.data.datasets[0].backgroundColor =
        materials.map((m) => MATERIAL_CHART_COLORS[m] || "#8c909f");
      charts.confidenceByMaterial.update();
    }

    // Summary stat cards
    if (els.analyticsAvgConfidence) {
      els.analyticsAvgConfidence.textContent = total
        ? roundTo(confidenceSum / total, 1) + "%"
        : "\u2014";
    }
    if (els.analyticsRecycleSplit) {
      els.analyticsRecycleSplit.textContent = total
        ? recyclableCount + " / " + (total - recyclableCount)
        : "\u2014";
    }
    if (els.analyticsTopMaterial) {
      els.analyticsTopMaterial.textContent = materials.length
        ? materials[0]
        : "\u2014";
    }
  }
  ...
  function updateSettingsConnectionBadge(connected) {
    ...
    els.settingsConnectionBadge.textContent = connected
      ? "Connected"
      : "Disconnected";
  ...
  function refreshBackendHealth() {
    if (els.settingsHealthStatus)
      els.settingsHealthStatus.textContent = "Checking\u2026";
    fetch(backendHttpUrl())
      .then((res) => res.json())
      .then((data) => {
        if (els.settingsHealthStatus)
          els.settingsHealthStatus.textContent = data.status || "OK";
        if (els.settingsModelStatus) {
          els.settingsModelStatus.textContent = data.model_loaded
            ? "Loaded"
            : "Not loaded";
        }
        if (els.settingsDashboardCount) {
          els.settingsDashboardCount.textContent =
            typeof data.connected_dashboards === "number"
              ? String(data.connected_dashboards)
              : "\u2014";
        }
      })
      .catch(() => {
        if (els.settingsHealthStatus)
          els.settingsHealthStatus.textContent = "Unreachable";
        if (els.settingsModelStatus)
  ...
  function wireSettingsPage() {
    ...
    if (els.settingsReconnectBtn) {
      els.settingsReconnectBtn.addEventListener("click", () => {
        reconnectWithUrl(
          els.settingsWsUrlInput
            ? els.settingsWsUrlInput.value
            : BACKEND_WS_URL,
        );
      });
    }
    if (els.settingsRefreshHealthBtn) {
      els.settingsRefreshHealthBtn.addEventListener(
        "click",
    ...
    if (els.settingsClearHistoryBtn) {
      els.settingsClearHistoryBtn.addEventListener("click", () => {
        if (
          !confirm(
            "Clear all sort history for this session? This cannot be undone.",
          )
        )
          return;
        historyLog = [];
  ...
  function buildTeamCard(member) {
    ...
    div.innerHTML =
      ...
      '<p class="font-body-md font-semibold text-on-surface truncate">' +
      escapeHtml(member.name) +
      "</p>" +
      '<p class="font-label-caps text-label-caps text-primary uppercase mt-0.5">' +
      escapeHtml(member.role) +
      "</p>" +
      '<p class="font-mono-data text-on-surface-variant break-all mt-1.5 text-[15px] leading-snug">' +
  ...
  function renderTeamCards() {
    ...
    TEAM_MEMBERS.forEach((member) =>
      els.supportTeamGrid.appendChild(buildTeamCard(member)),
    );
  }
  ...
  function openSupportModal(mode) {
    // mode: "chat" (right floating button) or "extras" (left sidebar button)
    ...
    if (els.supportChatSection)
      els.supportChatSection.style.display = showChat ? "" : "none";
    if (els.supportExtrasSection)
      els.supportExtrasSection.style.display = showChat ? "none" : "";

    if (showChat && els.supportChatInput) {
      els.supportChatInput.focus();
    } else if (els.complaintName) {
      els.complaintName.focus();
    }
  }
  ...
  function setComplaintStatus(kind, text) {
    ...
    els.complaintStatus.className =
      "font-body-md text-sm rounded-lg px-4 py-3 " +
      (styles[kind] || styles.error);
  ...
  function handleComplaintSubmit(event) {
    ...
    if (typeof emailjs === "undefined") {
      setComplaintStatus(
        "error",
        "Could not load the email service (EmailJS script blocked or offline).",
      );
      return;
    }
    ...
    emailjs
      ...
      .then(() => {
        setComplaintStatus(
          "success",
          "Thanks \u2014 your complaint was sent successfully.",
        );
      ...
      .catch((err) => {
        ...
        setComplaintStatus(
          "error",
          "Couldn't send your complaint. Please try again in a moment.",
        );
      })
      .finally(() => {
        els.complaintSubmitBtn.disabled = false;
        els.complaintSubmitBtn.classList.remove(
          "opacity-60",
          "cursor-not-allowed",
        );
      });
  ...
  function wireSupportModal() {
    ...
    if (els.sidebarSupportBtn)
      els.sidebarSupportBtn.addEventListener("click", () =>
        openSupportModal("extras"),
      );
    if (els.supportOpenBtn)
      els.supportOpenBtn.addEventListener("click", () =>
        openSupportModal("chat"),
      );
    if (els.supportCloseBtn)
      els.supportCloseBtn.addEventListener("click", closeSupportModal);
    ...
    if (els.complaintForm)
      els.complaintForm.addEventListener("submit", handleComplaintSubmit);
  }
      ...
      els.signInForm.addEventListener("submit", (event) => {
        ...
        attemptLogin(username, password)
          ...
          .catch((error) => {
            setSignInError(
              error.message ||
                "Could not reach the login server. Check the backend URL and try again.",
            );
          })
          .finally(() => {
  ...
  function wireNav() {
    ...
    function showView(name) {
      ...
      if (name === "analytics") {
        ...
        if (els.settingsWsUrlInput)
          els.settingsWsUrlInput.value = BACKEND_WS_URL;
        updateSettingsConnectionBadge(
